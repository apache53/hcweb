<div class="btn-group">
	<a href="<?php echo site_urlc('news/create')?>" class='btn btn-primary'> <i class="fa fa-plus"></i> <?php echo $title; ?> </a>
</div>
...
require(['adminer/js/ui'],function(ui){
	var news = {
		url_del: "<?php echo site_urlc('news/delete'); ?>"
		,url_audit: "<?php echo site_urlc('news/audit'); ?>"
		,url_flag: "<?php echo site_urlc('news/flag'); ?>"
		,url_sortid: "<?php echo site_urlc('news/sortid'); ?>"
		,url_sort_change: "<?php echo site_urlc('news/sort_change'); ?>"
		,url_copy: "<?php echo site_urlc('news/copypro'); ?>" 
	};
	ui.fancybox_img();
	ui.btn_delete(news.url_del);		// 删除
	ui.btn_audit(news.url_audit);	// 审核
	ui.btn_flag(news.url_flag);		// 推荐
	// ui.sortable(news.url_sortid);	// 排序  拖动排序和序号排序在firefox中有bug
	ui.sort_change(news.url_sort_change); // input 排序
	ui.btn_copy(news.url_copy);    // 热门审核
});
</script>
