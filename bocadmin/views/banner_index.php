<div class="btn-group">
	<a href="<?php echo site_urlc('banner/create')?>" class='btn btn-primary'> <i class="fa fa-plus"></i> <?php echo $title; ?> </a>
</div>
...
require(['adminer/js/ui'],function(ui){
	var banner = {
		url_del: "<?php echo site_urlc('banner/delete'); ?>"
		,url_audit: "<?php echo site_urlc('banner/audit'); ?>"
		,url_flag: "<?php echo site_urlc('banner/flag'); ?>"
		,url_sortid: "<?php echo site_urlc('banner/sortid'); ?>"
		,url_sort_change: "<?php echo site_urlc('banner/sort_change'); ?>"
		,url_copy: "<?php echo site_urlc('banner/copypro'); ?>" 
	};
	ui.fancybox_img();
	ui.btn_delete(banner.url_del);		// 删除
	ui.btn_audit(banner.url_audit);	// 审核
	ui.btn_flag(banner.url_flag);		// 推荐
	// ui.sortable(banner.url_sortid);	// 排序  拖动排序和序号排序在firefox中有bug
	ui.sort_change(banner.url_sort_change); // input 排序
	ui.btn_copy(banner.url_copy);    // 热门审核
});
</script>
